                ...
                <div className="content flex flex-col lg:flex-row mt-4 gap-x-5">  {/* flex */}
                    <div className="left bg-slate-50 lg:w-[30%]">
                        <div className="left-content p-2 h-full flex flex-col gap-y-4  lg:justify-evenly justify-start"> {/* flex  */}
                            <h2 className='text-3xl md:text-5xl font-bold capitalize '>innovating spaces, <br /> inspiring <br /> lives</h2>
                            <div className="image px-2 ">
                                <Image src={'/assets/feature1.jpg'} width={200} height={200} className='lg:w-full w-[80%] mx-auto rounded-lg' />
                            </div>
                            <div className="btn-para border-2 border-slate-300 rounded-lg px-2 py-1">
                                <p className='font-normal text-justify'>Lorem ipsum dolor sit amet consectetur adipisicing elit. Necessitatibus debitis eaque perspiciatis tempora veniam saepe! Nemo nostrum suscipit quos in aliquam amet tempora?</p>
                                <button className='rounded-lg border border-black py-2 mb-2 my-5 px-7 font-semibold capitalize'>learn more</button>
                    ...
                    <div className="right w-full border-l-2 border-slate-300">
                        <h1 className='capitalize text-4xl md:text-6xl text-center font-bold my-6'>feature project</h1>
                        <div className=' w-[80%] mx-auto'>
                            <div className="btn flex flex-col lg:flex-row gap-y-3 justify-around my-2 ">
                                {
                                    btnData.map((item, ind) => {
                                        return (
                                            <button key={ind} className='rounded-lg border border-black py-2 px-7 hover:bg-black hover:text-white duration-300 font-semibold capitalize'>{item}</button>
                                        )
                                    })
                                }
                            </div>
                                ...
                                {imgData.map(item => {
                                    return (
                                        <div key={item.cityName} className=" md:px-2 ">
                                            <Image src={item.imgSrc} width={480} height={480} className='rounded-lg h-72 lg:h-96 ' />
                                            <div className="text border rounded-lg px-2 py-1 my-2">
                                                <div className="row-1 flex justify-between items-center">
                                                    <p className='font-normal'>{item.projectName}</p>
